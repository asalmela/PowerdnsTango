...
any ['get', 'post'] => '/domains/edit/records/id/:id' => sub {
    ...
    {
        $sth = database->prepare(
            'select count(id) as count from records where domain_id = ? and type != ?'
        );
    ...
    $sth = database->prepare(
                      'select * from records where domain_id = ? and type = ?');
    $sth->execute($domain_id, 'SOA');
    my $soa = $sth->fetchrow_hashref;

    my ($name_server, $contact, undef, $refresh, $retry, $expire, $minimum) =
      (split /\s/, $soa->{content})
      if (defined $soa->{content});
    ...
    if (request->method() eq "POST" && $search ne '0')
    {
        ...
        flash error => "Record search found no match"
          if ($count->{'count'} == 0);
        flash message => "Record search found $count->{'count'} matches"
          if ($count->{'count'} >= 1);
    }
    else
    {
        $sth = database->prepare(
            'select * from records where domain_id = ? and type != ? limit ? offset ?'
        );
    ...
    if ($user_type eq 'admin')
    {
        ...
        my $check_owner = database->quick_select('domains_acl_tango',
                                                 {domain_id => $domain_id});
        ...
        {
            my $check_owner =
              database->quick_select('users_tango', {id => $owner_id});
    ...
    else
    {
        $templates = database->prepare(
            'select templates_tango.* from templates_tango, templates_acl_tango where (templates_tango.id = templates_acl_tango.template_id) and templates_acl_tango.user_id = ?'
        );
    ...
    template 'records',
      {
        ...
        templates          => $templates->fetchall_hashref('id'),
        page               => $load_page,
        ...
        minimum            => $minimum,
        ttl                => $soa->{ttl},
        users              => $users->fetchall_hashref('id'),
        domain_owner_id    => $owner_id,
        domain_owner_login => $owner_login
      };
};
...
post '/domains/edit/records/id/:id/update/domain' => sub {
    ...
    if (($count->{count} != 0) && ($old_domain->{name} ne $domain))
    {
        flash error => "Domain $domain already exists";
           ...
           && ($type =~ m/^NATIVE$/i || $type =~ m/^MASTER$/i))
    {
        ...
        database->quick_insert('domains_acl_tango',
                               {user_id => $owner_id, domain_id => $domain_id});
...
post '/domains/edit/records/id/:id/add' => sub {
    ...
    my ($stat, $message) =
      check_record($name, $ttl, $type, $content, $prio, 'live');
    ...
    my $sth = database->prepare(
        "select count(id) as count from records where domain_id = ? and type = ? and name = ?"
    );
    ...
    {
        flash error =>
          "Add record failed, CNAME record must be unique and contain a valid domain name";
...
post '/domains/edit/records/id/:id/add/template' => sub {
    ...
    if ($domain->{type} !~ m/^SLAVE$/i)
    {
        ...
        database->quick_delete('records', {domain_id => $domain_id});
        my $template =
          database->quick_select('templates_tango', {id => $template_id});
        my $templates_records = database->prepare(
                 'select * from templates_records_tango where template_id = ?');
        $templates_records->execute($template_id);

        while (my $template_row = $templates_records->fetchrow_hashref)
        {
            $template_row->{name} =~
              s/\%(\s)?(zone|domain|host)(\s)?\%/$domain->{name}/i;
        ...
        flash message =>
          "All records replaced using template $template->{name}";
    }
    else
    {
...
post '/domains/edit/records/id/:id/find/replace' => sub {
    ...
    {
        flash error =>
          "Failed to update records, ensure all fields are complete";
    ...
    if ($find_in eq 'ttl')
    {
        ...
        {
            flash error =>
              "Failed to update records, TTL must be a number equal or greater than $default_ttl_minimum->{value}";
    ...
    elsif ($find_in eq 'prio')
    {
        ...
        {
            flash error =>
              "Failed to update records, Priority must be a number";
    ...
    {
        my ($stat, $message) =
          check_record($domain, $default_ttl_minimum->{value},
                       $find_type, $replace, undef, 'live');

        if ($stat == 1)
        {
            flash error =>
              "Failed to update records, Content must match record type";
...
ajax '/domains/edit/records/update/soa' => sub {
    ...
    if ($domain_info->{type} =~ m/^SLAVE$/i)
    {
        return {
                stat    => 'fail',
                message => "SOA update failed, can't modify slave domain"
               };
    }

    my ($stat, $message) =
      check_soa($name_server, $contact, $refresh, $retry, $expire, $minimum,
                $ttl);
...
ajax '/domains/edit/records/get/record' => sub {
    ...
    my $domain =
      database->quick_select('domains', {id => $domain_id->{domain_id}});
...
ajax '/domains/edit/records/update/record' => sub {
    ...
    if ($domain->{type} =~ m/^SLAVE$/i)
    {
        return {
                stat    => 'fail',
                message => "Record update failed, can't modify slave domain"
               };
    }
    ...
    my $sth = database->prepare(
        "select count(id) as count from records where domain_id = ? and type = ? and name = ? and id != ?"
    );
    ...
    if ($type eq 'CNAME' && ($name eq $domain->{name} || $count->{count} != 0))
    {
        return {
            stat => 'fail',
            message =>
              "Failed to update record, CNAME record must be unique and contain a valid domain name"
        };
    }
